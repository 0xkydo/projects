...
const exchange_name = 'BobaSwap'; // TODO: fill in the name of your exchange

const token_name = 'GiaoToken';             // TODO: replace with name of your token
const token_symbol = 'GIAO';               // TODO: replace with symbol for your token
...
const token_address = '0xa9FdD8409171Cb99Dd4007F8498627f2476D1B29';                   
const token_abi = [
	{
		"inputs": [],
...
const exchange_address = '0x6CF5B51662D24C19dECdBB5130F55D4A0979aa58';    
const exchange_abi = [
	{
		"anonymous": false,
		"inputs": [
...
async function addLiquidity(amountEth, maxSlippagePct) {
    ...
    const total_supply = 10000000000;
    await token_contract.methods.approve(exchange_address,total_supply).send({from:web3.eth.defaultAccount, gas : 999999});
    //calculate for slippage
    let states = await getPoolState();
    var max_exchange_rate = Math.floor(states["token_eth_rate"] * (100-maxSlippagePct)/100 * _multiplier);
    var min_exchange_rate = Math.floor(states["token_eth_rate"] * (100+maxSlippagePct)/100 * _multiplier);
    //execution
    await exchange_contract.methods.addLiquidity(max_exchange_rate, min_exchange_rate).send({from:web3.eth.defaultAccount, value : amountEth, gas : 999999});
...
async function removeLiquidity(amountEth, maxSlippagePct) {
    ...
    var min_exchange_rate = Math.floor(states["token_eth_rate"]*  (100+maxSlippagePct) /100 * _multiplier);
    // execution
    await exchange_contract.methods.removeLiquidity(amountEth, max_exchange_rate, min_exchange_rate).send({from:web3.eth.defaultAccount, gas : 999999});
}
...
async function removeAllLiquidity(maxSlippagePct) {
    ...
    await exchange_contract.methods.removeAllLiquidity(max_exchange_rate,min_exchange_rate).send({from:web3.eth.defaultAccount, gas : 999999});
}
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
    /** TODO: ADD YOUR CODE HERE **/
    const total_supply = 10000000000;
    // call price, calculate max exchange rate.
    await token_contract.methods.approve(exchange_address,total_supply).send({from:web3.eth.defaultAccount, gas : 999999});
    //check slippage
    let states = await getPoolState();
    var max_exchange_rate = Math.floor(states["eth_token_rate"] * (100-maxSlippagePct)/100 * _multiplier);    
    //execution
...
async function swapETHForTokens(amountETH, maxSlippagePct) {
    ...
    //check slippage
    let states = await getPoolState();
	console.log(states["token_eth_rate"]);
	console.log(maxSlippagePct);
